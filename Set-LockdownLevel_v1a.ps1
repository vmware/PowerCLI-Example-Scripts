function Set-LockdownLevel {
    ...
    Process {
        ...
        Write-Host -Object "you are changing the lockdown mode on host [$VMhost] to $level" -ForegroundColor Yellow
        ...
        if ($filePath) {Write-Host -Object 'Saving current Lockdown Mode level to CSV' -ForegroundColor Yellow}

        #Retrieve the VMhost as a view object
        $lockdown = Get-View (Get-View -ViewType HostSystem -Filter @{
                'Name' = "$VMhost"
        ...
        {
            Write-Host -Object "Lockdown Mode Level Updated Successfully on host [$VMhost]" -ForegroundColor Green
        }
        else
        {
            Write-Host -Object "Uh Oh... Looks like the Lockdown Mode Level did not update for host [$VMhost]" -ForegroundColor Red
        }
        ...
        if ($filePath) {$Actions | Export-Csv -Path "$filePath" -NoTypeInformation -NoClobber -Append}
    }
}
